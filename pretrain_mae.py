...
def train_one_epoch(epoch, train_loader, model, optimizer, device):
    ...
    with tqdm(train_loader, leave=False) as pbar:
        for batch in pbar:
            ...
            optimizer.zero_grad()
            pred_images, mask = model(images)
            loss = torch.mean((pred_images - images) ** 2 * mask) / 0.75
            loss.backward()
            optimizer.step()
            ...
            results = {"train_loss": total_loss / total}
...
def evaluate(epoch, test_loader, model, device):
    ...
    with tqdm(test_loader, leave=False) as pbar:
        for batch in pbar:
            ...
            results = {"test_loss": total_loss / total}
            pbar.set_postfix(results)
...
def run(data_dir, device_list, batch_size, warmup_epochs, num_epochs):
    ...
    # MODEL
    model = MAE(img_size=32, patch_size=2, mask_ratio=0.75)
    ...
    with tqdm(range(num_epochs), leave=True) as pbar:
        for epoch in pbar:
            ...
            if loss < best_loss:
                ...
                torch.save(checkpoint, "./checkpoints/pretrain_mae_patch2_32.pth")
...
def visualize_reconstructions(test_dataset, model, device, steps):
    ...
    pred_images, mask = model(real_images)
    pred_images = pred_images * mask + real_images * (1 - mask)
    save_path = f"./images/mae_reconstructions_{steps}.png"
